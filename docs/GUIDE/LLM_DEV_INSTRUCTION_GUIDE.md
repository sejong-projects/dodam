...
description: "LLM 코딩 어시스턴트(Claude, Cursor, Codex, Gemini 등)에게 효과적으로 개발을 지시하기 위한 구조화된 지시서 작성법 가이드. 핵심 원칙, 도구별 형식 비교, 프롬프트 엔지니어링 기법, Spec-Driven Development, 실전 템플릿 및 MetaWorks 기반 작성 예시를 포함한다."
doc_type: "Guide"
version: "v1.0"
status: "Draft"
owner: "jhkim"
...
| **PRD** | 제품 요구사항 | 릴리스 사이클 | neo-metawork-PRD-v0.1.md |
...
- 추출된 각 테이블에 대해:
  → 컬럼 메타데이터를 추출한다
  → 데이터 타입을 MetaWorks 표준 타입으로 매핑한다
  → 메타데이터 레코드를 생성한다
```
...
### 10.1 NestJS 백엔드 프로젝트 지시서 (MetaWorks 기반)

MetaWorks 프로젝트의 백엔드 API에 대한 실전 CLAUDE.md 예시이다.
...
### 10.2 Nuxt/Vue 프론트엔드 프로젝트 지시서 (MetaWorks 기반)
...
### 10.3 모노레포 풀스택 프로젝트 지시서 (MetaWorks 기반)
...
| 항목 | ❌ 나쁜 지시서 | ✅ 좋은 지시서 |
|------|-------------|-------------|
| **프로젝트 설명** | "메타데이터 플랫폼" | "NestJS+Fastify 기반 B2C 메타데이터 관리 REST API. PostgreSQL 16, Prisma ORM 사용." |
...
- `docs/PRD/neo-metawork-PRD-v0.1.md` — MetaWorks PRD (기능 요구사항)
