# CLAUDE.md 품질 검증 체크리스트

> CLAUDE.md 작성 완료 후, 아래 체크리스트를 사용하여 품질을 검증하세요.
> EXCELLENT 등급 달성을 목표로 하며, 모든 항목을 충족할수록 AI 에이전트의 프로젝트 이해도가 높아집니다.

---

## 1. 프로젝트 개요

- [ ] 프로젝트 설명이 한 줄로 명확히 기술되어 있는가
- [ ] 기술 스택이 버전과 함께 명시되어 있는가
- [ ] 패키지 매니저가 명시되어 있는가
- [ ] 런타임(Node/Python/JVM 등) 버전이 명시되어 있는가

---

## 2. 프로젝트 구조

- [ ] 주요 디렉토리가 트리 형태로 표시되어 있는가
- [ ] 각 디렉토리의 역할이 설명되어 있는가
- [ ] 중요한 파일의 위치가 포함되어 있는가

---

## 3. 개발 규칙

- [ ] 네이밍 컨벤션이 코드 예시와 함께 있는가
- [ ] 사용하는 디자인 패턴이 코드로 시연되어 있는가
- [ ] 임포트 순서/규칙이 명시되어 있는가
- [ ] 추상적 표현("깨끗한 코드") 없이 구체적 규칙인가

---

## 4. 명령어

- [ ] 빌드, 테스트, 린트 명령어가 포함되어 있는가
- [ ] 명령어가 정확하고 실행 가능한가
- [ ] 환경별 명령어 차이가 있다면 명시되어 있는가

---

## 5. 워크플로우 규칙

- [ ] Git 브랜치 네이밍 규칙이 있는가
- [ ] 커밋 메시지 형식이 정의되어 있는가
- [ ] PR 프로세스가 기술되어 있는가

---

## 6. 경계 규칙

- [ ] Always Do / Ask First / Never Do가 분리되어 있는가
- [ ] 각 규칙이 구체적이고 실행 가능한가
- [ ] 규칙 간 모순이 없는가
- [ ] Never Do에 실제 위험한 작업이 포함되어 있는가

---

## 7. 테스트

- [ ] 테스트 실행 명령어가 포함되어 있는가
- [ ] 테스트 작성 패턴/규칙이 있는가
- [ ] 커버리지 기대치가 명시되어 있는가
- [ ] 테스트 프레임워크와 파일 명명 규칙이 명시되어 있는가 (예: Jest, *.spec.ts)

---

## 8. 의존성 및 도구

- [ ] 핵심 라이브러리와 사용 목적이 명시되어 있는가
- [ ] 외부 서비스와 해당 환경 변수명이 명시되어 있는가
- [ ] 개발 도구(린터, 포맷터 등)와 설정 파일 위치가 기재되어 있는가

---

## 9. 전체 품질

- [ ] 전체 길이가 300줄 이하인가 (초과 시 모듈화 고려)
- [ ] 산문 대신 코드 예시를 사용하고 있는가
- [ ] 모호한 표현("적절히", "깨끗하게")이 없는가
- [ ] 현재 프로젝트에 실제로 적용되는 규칙만 포함되어 있는가
- [ ] CLAUDE.md와 CLAUDE.local.md의 역할이 분리되어 있는가

---

## 점수 산정 (자체 평가)

| 카테고리 | 항목 수 | 충족 수 | 점수 |
| ------------------ | ------- | ------- | ---- |
| 프로젝트 개요 | 4 | /4 | |
| 프로젝트 구조 | 3 | /3 | |
| 개발 규칙 | 4 | /4 | |
| 명령어 | 3 | /3 | |
| 워크플로우 규칙 | 3 | /3 | |
| 경계 규칙 | 4 | /4 | |
| 테스트 | 4 | /4 | |
| 의존성 및 도구 | 3 | /3 | |
| 전체 품질 | 5 | /5 | |
| **합계** | **33** | **/33** | |

### 등급 기준

| 점수 | 등급 | 권장 조치 |
| ----- | ----------------- | -------------------------------- |
| 30~33 | EXCELLENT | 사용 가능 |
| 24~29 | GOOD | 누락 항목 보완 후 사용 |
| 19~23 | FAIR | 주요 섹션 보강 필요 |
| < 19 | INSUFFICIENT | 재작성 필요 |
